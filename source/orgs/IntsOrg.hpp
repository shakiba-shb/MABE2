...
namespace mabe {
  ...
  public:
    struct ManagerData : public Organism::ManagerData {
      ...
      int mut_size = 1.0;             ///< Standard deviation of mutations.
      int min_value = 0.0;            ///< Smallest that values are allowed to be.
      int max_value = 100.0;          ///< Largest that values are allowed to be.
      ...
      emp::Binomial mut_size_dist;       ///< Distribution of number of mutations to occur.     
    ...
    size_t Mutate(emp::Random & random) override {
      ...
      while (mut_pos != -1) {
        ...
        if (SharedData().mut_size > 0) {
          // std::cout << floor(random.GetNormal(0, SharedData().mut_size)) << std::endl;
          cur_val += floor(random.GetNormal(0, SharedData().mut_size));      // Mutate the value at the site.
        } else {
          cur_val += random.P(.5) ?  1 : -1;      // Mutate the value at the site.
        }
    ...
    void Randomize(emp::Random & random) override {
      ...
      double total = 0.0;
      for (double & x : vals) {
        x = (double)random.GetInt(SharedData().min_value, SharedData().max_value);
        total += x;
      }
      SetTrait<double>(SharedData().total_name, total);  // Store total in data map.
    }
    ...
    void SetupConfig() override {
      ...
      GetManager().LinkVar(SharedData().init_random, "init_random",
                      "Should we randomize ancestor?  (0 = all 0)");
    }
    ...
    void SetupModule() override {
      ...
      SharedData().mut_size_dist.Setup(.5, 50);
  ...
  void IntsOrg::ManagerData::ApplyBounds(double & value) {
    if (value > max_value) {
      switch (upper_bound) {
  ...
  void IntsOrg::ManagerData::ApplyBounds(std::span<double> & vals) {
  ...
  MABE_REGISTER_ORG_TYPE(IntsOrg, "Organism consisting of a series of N integer values.");
}
